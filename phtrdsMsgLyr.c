...
void updateArea( int pos, int capacity, int slotsAvailable, int zone, Area *parking) {
  if ( pos < AREA_LOCS ) {
    parking[pos].capacity = capacity ;
    parking[pos].slotsAvailable = slotsAvailable ;
    parking[pos].zone = zone ;
  }
}